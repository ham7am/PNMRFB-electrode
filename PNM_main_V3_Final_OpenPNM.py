"""
Main script rewritten into OpenPNM V3.0.
...
Working principle:
    ...
    3. Create, initialize and run stokesflow algorithms.
                                
    4. Create and initialize mass and charge transport algorithms with 
       associated physics models and ascribe phase properties. 
    
    5. Run mass and charge transport iterative algorithm
        a. The mass and charge transport algorithms are ran sequentially in 
           each half-cell. The two half-cells are coupled via the membrane 
           potential.
           
        b. Checking convergence of the simulation.
        
        c. Writing the converged simulation results to:
            1. A .VTP file for visualization in Paraview.
            2. A .CSV file for containing per network the:
                     a. Current density at applied cell voltageanodic and cathodic 
                     b. Andodic and cathodic activation, concentration and ohmic overpotentials
          The data is saved in a folder placed under the output 
          folder. Make sure that before starting the simulation, YOU have created
          this folder (i.e. its name should be equal to the string set in Output_folder in step 1).
...
One can apply the fitting factors given below in this script. If fitting is
*not* required set these fitting factors to unity (1).     
    ...
    2. Rm_factor            -> Membrane resistance fiting factor.
"""
...
#------------------------------Fitting parameters-----------------------------#
# Set fitting factors to unity (1) if not used
km_factor =  1.0                
Rm_factor = 1.0
...
for NrNetwork, file in enumerate(file_vec):
    for v_in in v_in_vec:
        ...
        if Flow_field == 1:
            ...
            cf.assign_boundary_pores_IDFF(net_a, W_dim, L_dim, H_dim, H_a, H_a_min, 0)
        ...
        v_in_c = v_in ; v_in_a = v_in
        Q_in_c = v_in_c * A_in_c ; Q_in_a = v_in_a * A_in_a         
        ...
        if Flow_field == 0:
            ...
            # Finding the outlet throats of the even and odd networks 
            ...
            total_area_inlet_a = np.sum(cross_area_a[net_a.pores('flow_inlet')])
            total_area_outlet_a = np.sum(cross_area_a[net_a.pores('flow_outlet')])   
            ...
            for pore in net_c.pores('flow_outlet'):
                sf_c_even.set_rate_BC(rates=Q_in_c * cross_area_c[pore] / total_area_outlet_c, pores=pore)      # Neumann inlet boundary condition
            
            for pore in net_a.pores('flow_inlet'):
                sf_a_odd.set_rate_BC(rates=Q_in_a * cross_area_a[pore] / total_area_inlet_a, pores=pore)        # Neumann inlet boundary condition
            
            for pore in net_a.pores('flow_outlet'):
                sf_a_even.set_rate_BC(rates=Q_in_a * cross_area_a[pore] / total_area_outlet_a, pores=pore)      # Neumann inlet boundary condition        
        ...
        if Flow_field == 1:
            ...
            cross_area_a = np.pi / 4 * net_a['pore.diameter'] ** 2
            total_area_inlet_c = np.sum(cross_area_c[net_c.pores('flow_inlet')])
            total_area_outlet_c = np.sum(cross_area_c[net_c.pores('flow_inlet')])
            total_area_inlet_a = np.sum(cross_area_a[net_a.pores('flow_inlet')])
            total_area_outlet_a = np.sum(cross_area_a[net_a.pores('flow_inlet')]) 
            ...
            Pout = 0                                                                 # Pressure outlet boundary condition [Pa]
            sf_c_odd.set_value_BC(values=Pout, pores=net_c.pores('flow_outlet'))     # Dirichlet outlet boundary condition
            sf_c_even.set_value_BC(values=Pout, pores=net_c.pores('flow_outlet'))    # Dirichlet outlet boundary condition
            sf_a_odd.set_value_BC(values=Pout, pores=net_a.pores('flow_outlet'))     # Dirichlet outlet boundary condition
            sf_a_even.set_value_BC(values=Pout, pores=net_a.pores('flow_outlet'))    # Dirichlet outlet boundary condition
        ...
        # Set initial guesses to the mean pressure found above (for fsolve)
        if Flow_field == 0:
        ...
        if Flow_field == 1:
            x0_c_even = catholyte['pore.pressure'][net_c.pores('flow_inlet')].mean()
            x0_a_even = anolyte['pore.pressure'][net_a.pores('flow_inlet')].mean() 
        
        # Delete the initial guess Neumann (flow rate) boundary condition
        sf_c_odd.clear_rate_BCs()
        sf_c_even.clear_rate_BCs()
        sf_a_odd.clear_rate_BCs()
        sf_a_even.clear_rate_BCs()     
        
        # Find the pressure at the inlet at which the total flow rate matches 
        # the desired flow rate. Aditionally, compute and extract throat and 
        # pore velocities to safe in stokesflow algorithm and network.
        if Flow_field == 0:
            ...
            optimize.fsolve(func=cf.inlet_pressure_V3, x0=x0_a_odd, args=(sf_a_odd, Q_in_a, inlet_throats_a_odd, net_a.pores('flow_inlet'), net_a, anolyte, net_a.pores('flow_outlet')))
            anolyte.update(sf_a_odd.soln)
            cf_pres_fit.Throat_pore_velocity_extraction_OpenPNM(net_a, 1, sf_a_odd)
            optimize.fsolve(func=cf.inlet_pressure_V3, x0=x0_c_even, args=(sf_c_even, Q_in_c, inlet_throats_c_even, net_c.pores('flow_outlet'), net_c, catholyte,  net_c.pores('flow_inlet')))
            catholyte.update(sf_c_even.soln)
            cf_pres_fit.Throat_pore_velocity_extraction_OpenPNM(net_c, 0, sf_c_even)
            optimize.fsolve(func=cf.inlet_pressure_V3, x0=x0_a_even, args=(sf_a_even, Q_in_a, inlet_throats_a_even, net_a.pores('flow_outlet'), net_a, anolyte, net_a.pores('flow_inlet')))
            anolyte.update(sf_a_even.soln)
            cf_pres_fit.Throat_pore_velocity_extraction_OpenPNM(net_a, 0, sf_a_even)
        if Flow_field == 1:
            optimize.fsolve(func=cf.inlet_pressure_V3, x0=x0_c_even, args=(sf_c_even, Q_in_c, inlet_throats_c_even, net_c.pores('flow_inlet'), net_c, catholyte,  net_c.pores('flow_outlet')))
            catholyte.update(sf_c_even.soln)
            cf_pres_fit.Throat_pore_velocity_extraction_OpenPNM(net_c, 0, sf_c_even)
            optimize.fsolve(func=cf.inlet_pressure_V3, x0=x0_a_even, args=(sf_a_even, Q_in_a, inlet_throats_a_even, net_a.pores('flow_inlet'), net_a, anolyte, net_a.pores('flow_outlet')))
            anolyte.update(sf_a_even.soln)
            cf_pres_fit.Throat_pore_velocity_extraction_OpenPNM(net_a, 0, sf_a_even)
            optimize.fsolve(func=cf.inlet_pressure_V3, x0=x0_c_odd, args=(sf_c_odd, Q_in_c, inlet_throats_c_odd, net_c.pores('flow_inlet'), net_c, catholyte, net_c.pores('flow_outlet')))
            catholyte.update(sf_c_odd.soln)
        ...
        # Initializing simulation with the odd network fluid field
        cf_pres_fit.Update_phase_with_SF_OpenPNM(sf_c_odd, catholyte, 1)
        cf_pres_fit.Update_phase_with_SF_OpenPNM(sf_a_odd, anolyte, 1)

        # Compute and update phase and associated algorithms with mass transfer coefficient
        cf_pres_fit.Mass_transfer_coefficient_OpenPNM(net_c, catholyte, 1, Velocity_dependent_km, param)
        cf_pres_fit.Mass_transfer_coefficient_OpenPNM(net_a, anolyte, 1, Velocity_dependent_km, param)
        ...
        # the function cf.bv_rate_constant_ad_c/a and is the non-linearized
            ...
            for network in range(number_of_networks):
                ...
                #------------------ Cathodic half-cell -----------------------#
                    ...
                    if (network + 1) % 2 == 0:      # Even networks
                        ...
                        catholyte.update(sf_c_even.soln)
                        cf_pres_fit.Update_phase_with_SF_OpenPNM(sf_c_even, catholyte, 0)
                        
                        # Local MT coefficient
                        cf_pres_fit.Mass_transfer_coefficient_OpenPNM(net_c, catholyte, 0, Velocity_dependent_km, param)
                  ...
                  #------------------ Anodic half-cell -----------------------#
                    ...
                    if (network + 1) % 2 == 0:
                        # Retrieve the StokesFlow data for an even network
                        anolyte.update(sf_a_even.soln)
                        cf_pres_fit.Update_phase_with_SF_OpenPNM(sf_a_even, anolyte, 0)
                        
                        # Local MT coefficient
                        cf_pres_fit.Mass_transfer_coefficient_OpenPNM(net_a, anolyte, 0, Velocity_dependent_km, param)
